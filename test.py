...
TEST_CASES = [
    TestCase(
        name="NAND Gate",
        verilog_file="parts/nand_gate.sv",
        test_file="parts/nand_gate.json",
    ...
    TestCase(
        name="Inverter",
        verilog_file="parts/inverter.sv",
        test_file="parts/inverter.json",
    ...
    TestCase(
        name="AND Gate",
        verilog_file="parts/and_gate.sv",
        test_file="parts/and_gate.json",
    ...
    TestCase(
        name="OR Gate",
        verilog_file="parts/or_gate.sv",
        test_file="parts/or_gate.json",
    ...
    TestCase(
        name="XOR Gate",
        verilog_file="parts/xor_gate.sv",
        test_file="parts/xor_gate.json",
    ...
    TestCase(
        name="NOR Gate",
        verilog_file="parts/nor_gate.sv",
        test_file="parts/nor_gate.json",
    ...
    TestCase(
        name="Half Adder",
        verilog_file="parts/half_adder.sv",
        test_file="parts/half_adder.json",
    ...
    TestCase(
        name="Full Adder",
        verilog_file="parts/full_adder.sv",
        test_file="parts/full_adder.json",
    ...
    TestCase(
        name="1-bit ALU",
        verilog_file="parts/alu_1bit.sv",
        test_file="parts/alu_1bit.json",
    ...
    TestCase(
        name="4-bit Adder",
        verilog_file="parts/adder_4bit_bus.sv",
        test_file="parts/adder_4bit_bus.json",
    ...
    TestCase(
        name="Demux 1-to-2",
        verilog_file="parts/demux_1to2.sv",
        test_file="parts/demux_1to2.json",
    ...
    TestCase(
        ...
        verilog_file="parts/mux_4to1.sv",
        test_file="parts/mux_4to1.json",
    ...
    def generate_report(self) -> str:
        ...
        for i, result in enumerate(self.results, 1):
            ...
            if result.inputs_parsed:
                ...
                report_lines.append(
                    f"**Inputs Parsed:** {result.inputs_parsed} {inputs_emoji}"
                )
                if not inputs_match:
                    report_lines.append(
                        f"  - Expected: {result.test_case.expected_inputs}"
                    )

            if result.outputs_parsed:
                outputs_match = (
                    result.outputs_parsed == result.test_case.expected_outputs
                )
                outputs_emoji = "✅" if outputs_match else "❌"
                report_lines.append(
                    f"**Outputs Parsed:** {result.outputs_parsed} {outputs_emoji}"
                )
            ...
            if result.test_case.test_file:
                if result.tests_total > 0:
                    test_emoji = (
                        "✅" if result.tests_passed == result.tests_total else "❌"
                    )
                    report_lines.append(
                        f"**Test Cases:** {result.tests_passed}/{result.tests_total} passed {test_emoji}"
                    )
                else:
                    report_lines.append(f"**Test Cases:** No test results found ❌")
            else:
                report_lines.append(
                    f"**Test Cases:** No test file specified (truth table only)"
                )
